...
{
	CKLOG(string("Loading shaders ") + vert + string(" and ") + frag,1);
	// Loading the files
...
void ShaderManager :: loadShaderFile(const std::string& shaderfile)
{
	...
	//Parse the uniform data
	parsec = parser.getSection("Uniform");
	if(parsec)
	{
		vector<const ParserSection *> uninames = parsec->getChildren();
		for(size_t i=0;i<uninames.size();++i)
		{
			Uniform uni;
			// Get location
			uni.parseConfiguration(uninames[i]);
			uni.set_loc(glGetUniformLocation(_active_program,uni.nameInShader().c_str()));
			_shaders[last]->uniform().push_back(uni);
		}
	}
	...
	if(parsec)
	{
		...
		{
			string nis = attnames[i]->getVal("AttributeRef");
			assert(nis != "");
			unsigned j;
