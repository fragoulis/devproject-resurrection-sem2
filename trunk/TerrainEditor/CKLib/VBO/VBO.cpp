...
void VBO :: setup(const std::vector<const VertexAttribute *>& vattrs)
{
	...
	_ifreeSpace.push_back(vec2<unsigned>(0,_indexSize));
	_dfreeSpace.push_back(vec2<unsigned>(0,_dataSize));
...
void VBO :: call(const unsigned start,const unsigned size,const unsigned drawmode)
{
	...
	VAttribStatus cur_flags = VBOMgr::instance()->getCurrentAttribs().AND(_potentialAttribs);
...
bool VBO :: querySetData(const unsigned d_size,const unsigned i_size,unsigned& d_pos,unsigned& i_pos)
{
	...
	vector<vec2<unsigned> >::iterator iti = _ifreeSpace.begin();
	...
	if(ok_one)
	{
		vector<vec2<unsigned> >::iterator itd = _dfreeSpace.begin();
	...
	for(unsigned i=0;i<_ifreeSpace.size();++i)
	{
		if( _ifreeSpace[i].x() > i_bound)
		{
			if(i && _ifreeSpace[i-1].y() == i_pos)
				...
				_ifreeSpace.insert(_ifreeSpace.begin() + i,vec2<unsigned>(i_pos,i_pos+i_size));
			break;
		}
	...
	for(unsigned i=0;i<_dfreeSpace.size();++i)
	{
		if( _dfreeSpace[i].x() > d_bound)
		{
			if(i && _dfreeSpace[i-1].y() == d_pos)
				...
				_dfreeSpace.insert(_dfreeSpace.begin() + i,vec2<unsigned>(d_pos,d_pos+d_size));
			return;
		}
