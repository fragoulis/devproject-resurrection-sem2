...
void Crater :: update(float dt)
{
	...
	if (m_timeTillNextEvent < 0.0f && !isProtected() && m_protectorType != 0)
	{
		...
		m_protector = GameLogic::instance().spawnEnemy(m_protectorType, m_protectorEnergyType, pos.getX(), pos.getZ(), true);
		if (m_protectorCarrierEnergyTypeSpawns != ENERGY_TYPE_UNKNOWN)
		{
			EnemyCarrier* carrier = static_cast<EnemyCarrier*>(m_protector);
