...
PS_MultiTexExplosion :: PS_MultiTexExplosion(const std::string& name,
			VBO * vbo,
			const float psize,
			const float syslife,
			const float plife,
...
{
	// Initialize
	_generateData(vbo,TextureIO::instance()->getTexture(texture), TextureIO::instance()->getTexture(shapeTexture));
}
...
PS_Base * PS_MultiTexExplosion :: clone() const
{
	return new PS_MultiTexExplosion(m_nameId,m_quadArray,m_particleSize,m_systemLife,m_particleLife,m_particleNum,
					 m_shaderIndex,m_usedAttribs);
}


void  PS_MultiTexExplosion :: _generateData(VBO * vbo,Texture * tex, Texture * shapeTex)
{
...
void PS_MultiTexExplosion :: update(const float delta)
{
	m_currentTime += delta;
}

void PS_MultiTexExplosion :: render() const
{
	...
	CHECK_GL_ERROR();
	ShaderManager::instance()->setUniform1i("shapeTex",1);
	CHECK_GL_ERROR();
