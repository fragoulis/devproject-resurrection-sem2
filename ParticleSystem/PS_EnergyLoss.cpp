...
PS_EnergyLoss :: PS_EnergyLoss(const std::string& name,
			VBO * vbo,
			const float psize,
			const float syslife,
			const float plife,
...
void  PS_EnergyLoss :: _generateData(VBO * vbo,Texture * tex)
{
	...
	for(unsigned i=0; i<totalVData ;i+=4)
	{
		...
		vel.normalize();
		vel.multiply(1.0f + RandomGenerator::GET_RANDOM_FLOAT(0.5f,4.0f));
